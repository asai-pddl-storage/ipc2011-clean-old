...
(define (domain transport-strips) (:requirements :typing :action-costs)
 (:predicates (at ?o - locatable ?l - location) (sum ?a ?b ?c - fuellevel)
  (fuel ?t - truck ?level - fuellevel)
  (fuelcost ?level - fuellevel ?l1 ?l2 - location) (in ?p - package ?t - truck)
  (connected ?l1 ?l2 - location))
 (:types location fuellevel locatable - object package truck - locatable)
 (:functions (total-cost) - number)
 (:action ugly11099ugly11100ugly11101ugly11099ugly2ugly1ugly3ugly2load
  :parameters (?p - package ?t - truck ?l - location) :precondition
  (and (at ?t ?l) (at ?p ?l)) :effect (and (not (at ?p ?l)) (in ?p ?t)))
 (:action ugly11100ugly11101ugly11099ugly11100ugly3ugly2ugly1ugly3unload
  :parameters (?p - package ?t - truck ?l - location) :precondition
  (and (at ?t ?l) (in ?p ?t)) :effect (and (at ?p ?l) (not (in ?p ?t))))
 (:action ugly11101ugly11099ugly11100ugly11101ugly1ugly3ugly2ugly1drive
  :parameters
