...
(define (domain miconic) (:requirements :strips :action-costs)
 (:predicates (above ?floor1 ?floor2) (lift-at ?floor) (destin ?person ?floor)
  (floor ?floor) (passenger ?passenger) (served ?person)
  (origin ?person ?floor) (boarded ?person))
 (:action ugly1up :parameters (?f1 ?f2) :precondition
  (and (floor ?f1) (floor ?f2) (lift-at ?f1) (above ?f1 ?f2)) :effect
  (and (lift-at ?f2) (not (lift-at ?f1)) (increase (total-cost) 1)))
 (:action ugly2board :parameters (?f ?p) :precondition
  (and (floor ?f) (passenger ?p) (lift-at ?f) (origin ?p ?f)) :effect
  (boarded ?p))
 (:action ugly3depart :parameters (?f ?p) :precondition
  (and (floor ?f) (passenger ?p) (lift-at ?f) (destin ?p ?f) (boarded ?p))
  :effect (and (not (boarded ?p)) (served ?p)))
 (:action ugly4down :parameters (?f1 ?f2) :precondition
