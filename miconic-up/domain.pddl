...
(define (domain miconic) (:requirements :strips :action-costs)
 (:predicates (destin ?person ?floor) (served ?person) (lift-at ?floor)
  (origin ?person ?floor) (passenger ?passenger) (above ?floor1 ?floor2)
  (boarded ?person) (floor ?floor))
 (:action
  ugly11091ugly11093ugly11091ugly11093ugly11091ugly3ugly2ugly1ugly3depart
  :parameters (?f ?p) :precondition
  (and (floor ?f) (passenger ?p) (lift-at ?f) (destin ?p ?f) (boarded ?p))
  :effect (and (not (boarded ?p)) (served ?p)))
 (:action
  ugly11092ugly11092ugly11092ugly11092ugly11092ugly2ugly1ugly3ugly2board
  :parameters (?f ?p) :precondition
  (and (floor ?f) (passenger ?p) (lift-at ?f) (origin ?p ?f)) :effect
  (boarded ?p))
 (:action ugly11093ugly11091ugly11093ugly11091ugly11093ugly1ugly3ugly2ugly1up
 ...
 (:action ugly11094ugly11094ugly11094ugly11094ugly11094ugly4ugly4ugly4ugly4down
  :parameters (?f1 ?f2) :precondition
