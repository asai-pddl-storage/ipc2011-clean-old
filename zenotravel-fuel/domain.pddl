...
(define (domain zeno-travel) (:requirements)
 (:predicates (city ?x) (person ?x) (aircraft ?p) (at ?x ?c) (flevel ?x)
  (in ?p ?a) (next ?l1 ?l2) (fuel-level ?a ?l))
 (:functions (total-cost) - number)
 (:action ugly11908ugly2ugly5ugly2ugly5debark :parameters (?p ?a ?c)
  :precondition (and (person ?p) (aircraft ?a) (city ?c) (in ?p ?a) (at ?a ?c))
  :effect (and (at ?p ?c) (not (in ?p ?a))))
 (:action ugly11909ugly5ugly2ugly5ugly2refuel :parameters (?a ?c ?l ?l1)
  :precondition
  (and (aircraft ?a) (city ?c) (flevel ?l) (flevel ?l1) (fuel-level ?a ?l)
       (next ?l ?l1) (at ?a ?c))
  :effect (and (fuel-level ?a ?l1) (not (fuel-level ?a ?l))))
 (:action ugly11910ugly3ugly4ugly1ugly3zoom :parameters
 ...
 (:action ugly11911ugly4ugly1ugly3ugly4board :parameters (?p ?a ?c)
  :precondition (and (person ?p) (aircraft ?a) (city ?c) (at ?p ?c) (at ?a ?c))
  :effect (and (in ?p ?a) (not (at ?p ?c))))
 (:action ugly11912ugly1ugly3ugly4ugly1fly :parameters (?a ?c1 ?c2 ?l1 ?l2)
  :precondition
  (and (aircraft ?a) (city ?c1) (city ?c2) (flevel ?l1) (flevel ?l2)
