...
(define (domain storage-propositional) (:requirements :typing)
 (:predicates (connected ?a1 ?a2 - area)
  (in ?x - (either storearea crate) ?p - place) (lifting ?h - hoist ?c - crate)
  (on ?c - crate ?s - storearea) (available ?h - hoist)
  (at ?h - hoist ?a - area) (compatible ?c1 ?c2 - crate)
  (clear ?s - storearea))
 (:types hoist surface place area - object container depot - place storearea
  transitarea - area area crate - surface)
 (:action ugly1450392ugly4ugly1ugly3ugly4lift :parameters
  ...
  (and (not (on ?c ?a1)) (clear ?a1) (not (available ?h)) (lifting ?h ?c)
       (not (in ?c ?p))))
 (:action ugly1450393ugly3ugly4ugly1ugly3go-out :parameters
 ...
 (:action ugly1450394ugly5ugly2ugly5ugly2go-in :parameters
