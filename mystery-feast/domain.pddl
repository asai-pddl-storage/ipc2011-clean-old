...
(define (domain mystery-strips) (:requirements :strips :action-costs)
 (:predicates (craves ?v ?n) (orbits ?i ?j) (eats ?n1 ?n2) (food ?x)
  (pleasure ?x) (harmony ?v ?s) (province ?x) (attacks ?i ?j) (fears ?c ?v)
  (pain ?x) (locale ?n ?a) (planet ?x))
 (:functions (total-cost) - number)
 (:action ugly24ugly25ugly24ugly2ugly3ugly2ugly3feast :parameters
 ...
 (:action ugly25ugly24ugly25ugly1ugly1ugly1ugly1overcome :parameters
  ...
  (and (not (craves ?c ?n)) (fears ?c ?v) (not (harmony ?v ?s2))
       (harmony ?v ?s1)))
 (:action ugly26ugly26ugly26ugly3ugly2ugly3ugly2succumb :parameters
  (?c ?v ?n ?s1 ?s2) :precondition
