...
(define (domain mystery-strips) (:requirements :strips :action-costs)
 (:predicates (attacks ?i ?j) (province ?x) (craves ?v ?n) (harmony ?v ?s)
  (pleasure ?x) (food ?x) (fears ?c ?v) (locale ?n ?a) (orbits ?i ?j)
  (planet ?x) (pain ?x) (eats ?n1 ?n2))
 (:functions (total-cost) - number)
 (:action ugly24ugly25ugly1ugly1ugly1ugly1overcome :parameters
  ...
  (and (not (craves ?c ?n)) (fears ?c ?v) (not (harmony ?v ?s2))
       (harmony ?v ?s1)))
 (:action ugly25ugly24ugly2ugly3ugly2ugly3feast :parameters
 ...
 (:action ugly26ugly26ugly3ugly2ugly3ugly2succumb :parameters
  (?c ?v ?n ?s1 ?s2) :precondition
