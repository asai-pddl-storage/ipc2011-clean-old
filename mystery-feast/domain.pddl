...
(define (domain mystery-strips) (:requirements :strips :action-costs)
 (:predicates (locale ?n ?a) (fears ?c ?v) (attacks ?i ?j) (food ?x)
  (pleasure ?x) (harmony ?v ?s) (orbits ?i ?j) (pain ?x) (province ?x)
  (eats ?n1 ?n2) (planet ?x) (craves ?v ?n))
 (:functions (total-cost) - number)
 (:action ugly24ugly2ugly3ugly2ugly3feast :parameters (?v ?n1 ?n2 ?l1 ?l2)
  :precondition
 ...
 (:action ugly25ugly1ugly1ugly1ugly1overcome :parameters (?c ?v ?n ?s1 ?s2)
  :precondition
  ...
  (and (not (craves ?c ?n)) (fears ?c ?v) (not (harmony ?v ?s2))
       (harmony ?v ?s1)))
 (:action ugly26ugly3ugly2ugly3ugly2succumb :parameters (?c ?v ?n ?s1 ?s2)
  :precondition
