...
(define (domain zeno-travel) (:requirements)
 (:predicates (aircraft ?p) (flevel ?x) (next ?l1 ?l2) (in ?p ?a)
  (fuel-level ?a ?l) (city ?x) (at ?x ?c) (person ?x))
 (:action ugly1fly :parameters (?a ?c1 ?c2 ?l1 ?l2) :precondition
  ...
  (and (at ?a ?c2) (fuel-level ?a ?l2) (not (at ?a ?c1))
       (not (fuel-level ?a ?l1))))
 (:action ugly2refuel :parameters (?a ?c ?l ?l1) :precondition
  (and (aircraft ?a) (city ?c) (flevel ?l) (flevel ?l1) (fuel-level ?a ?l)
       (next ?l ?l1) (at ?a ?c))
  :effect (and (fuel-level ?a ?l1) (not (fuel-level ?a ?l))))
 (:action ugly3zoom :parameters (?a ?c1 ?c2 ?l1 ?l2 ?l3) :precondition
  (and (aircraft ?a) (city ?c1) (city ?c2) (flevel ?l1) (flevel ?l2)
  ...
  (and (at ?a ?c2) (fuel-level ?a ?l3) (not (at ?a ?c1))
       (not (fuel-level ?a ?l1))))
 (:action ugly4board :parameters (?p ?a ?c) :precondition
  (and (person ?p) (aircraft ?a) (city ?c) (at ?p ?c) (at ?a ?c)) :effect
  (and (in ?p ?a) (not (at ?p ?c))))
 (:action ugly5debark :parameters (?p ?a ?c) :precondition
