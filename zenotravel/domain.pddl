...
(define (domain zeno-travel) (:requirements)
 (:predicates (in ?p ?a) (fuel-level ?a ?l) (next ?l1 ?l2) (at ?x ?c)
  (person ?x) (city ?x) (aircraft ?p) (flevel ?x))
 (:action ugly1450448ugly5ugly2ugly5ugly2refuel :parameters (?a ?c ?l ?l1)
  :precondition
  (and (aircraft ?a) (city ?c) (flevel ?l) (flevel ?l1) (fuel-level ?a ?l)
       (next ?l ?l1) (at ?a ?c))
  :effect (and (fuel-level ?a ?l1) (not (fuel-level ?a ?l))))
 (:action ugly1450449ugly4ugly1ugly3ugly4board :parameters (?p ?a ?c)
  :precondition (and (person ?p) (aircraft ?a) (city ?c) (at ?p ?c) (at ?a ?c))
  :effect (and (in ?p ?a) (not (at ?p ?c))))
 (:action ugly1450450ugly1ugly3ugly4ugly1fly :parameters (?a ?c1 ?c2 ?l1 ?l2)
  :precondition
  ...
  (and (at ?a ?c2) (fuel-level ?a ?l2) (not (at ?a ?c1))
       (not (fuel-level ?a ?l1))))
 (:action ugly1450451ugly2ugly5ugly2ugly5debark :parameters (?p ?a ?c)
  :precondition (and (person ?p) (aircraft ?a) (city ?c) (in ?p ?a) (at ?a ?c))
  :effect (and (at ?p ?c) (not (in ?p ?a))))
 (:action ugly1450452ugly3ugly4ugly1ugly3zoom :parameters
