...
(define (domain zeno-travel) (:requirements)
 ...
 (:action ugly1ugly3ugly4ugly1fly :parameters (?a ?c1 ?c2 ?l1 ?l2)
  :precondition
  ...
  (and (at ?a ?c2) (fuel-level ?a ?l2) (not (at ?a ?c1))
       (not (fuel-level ?a ?l1))))
 (:action ugly2ugly5ugly2ugly5debark :parameters (?p ?a ?c) :precondition
  (and (person ?p) (aircraft ?a) (city ?c) (in ?p ?a) (at ?a ?c)) :effect
  (and (at ?p ?c) (not (in ?p ?a))))
 (:action ugly3ugly4ugly1ugly3zoom :parameters (?a ?c1 ?c2 ?l1 ?l2 ?l3)
  :precondition
  (and (aircraft ?a) (city ?c1) (city ?c2) (flevel ?l1) (flevel ?l2)
  ...
  (and (at ?a ?c2) (fuel-level ?a ?l3) (not (at ?a ?c1))
       (not (fuel-level ?a ?l1))))
 (:action ugly4ugly1ugly3ugly4board :parameters (?p ?a ?c) :precondition
  (and (person ?p) (aircraft ?a) (city ?c) (at ?p ?c) (at ?a ?c)) :effect
  (and (in ?p ?a) (not (at ?p ?c))))
 (:action ugly5ugly2ugly5ugly2refuel :parameters (?a ?c ?l ?l1) :precondition
