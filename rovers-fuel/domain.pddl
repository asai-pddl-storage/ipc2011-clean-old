...
(define (domain rover) (:requirements :typing :action-costs)
 (:predicates (at_rock_sample ?w - waypoint)
  (have_rock_analysis ?r - rover ?w - waypoint)
  (can_traverse ?r - rover ?x - waypoint ?y - waypoint)
  (equipped_for_rock_analysis ?r - rover)
  (visible_from ?o - objective ?w - waypoint) (supports ?c - camera ?m - mode)
  (visible ?w - waypoint ?p - waypoint) (communicated_rock_data ?w - waypoint)
  (equipped_for_imaging ?r - rover) (at ?x - rover ?y - waypoint)
  (calibrated ?c - camera ?r - rover) (equipped_for_soil_analysis ?r - rover)
  (communicated_image_data ?o - objective ?m - mode) (full ?s - store)
  (have_image ?r - rover ?o - objective ?m - mode) (available ?r - rover)
  (at_soil_sample ?w - waypoint) (at_lander ?x - lander ?y - waypoint)
  (have_soil_analysis ?r - rover ?w - waypoint)
  (calibration_target ?i - camera ?o - objective)
  (communicated_soil_data ?w - waypoint) (empty ?s - store)
  (store_of ?s - store ?r - rover) (on_board ?i - camera ?r - rover)
  (channel_free ?l - lander))
 (:types rover waypoint store camera mode lander objective)
 (:functions (total-cost) - number)
 (:action ugly1ugly2ugly4ugly5communicate_rock_data :parameters
 ...
 (:action ugly2ugly4ugly5ugly8navigate :parameters
 ...
 (:action ugly3ugly9ugly3ugly9sample_rock :parameters
 ...
 (:action ugly4ugly5ugly8ugly1sample_soil :parameters
  (?x - rover ?s - store ?p - waypoint) :precondition
 ...
 (:action ugly5ugly8ugly1ugly2drop :parameters (?x - rover ?y - store)
  :precondition (and (store_of ?y ?x) (full ?y)) :effect
  (and (not (full ?y)) (empty ?y)))
 (:action ugly6ugly6ugly6ugly6take_image :parameters
 ...
 (:action ugly7ugly7ugly7ugly7communicate_soil_data :parameters
  (?r - rover ?l - lander ?p - waypoint ?x - waypoint ?y - waypoint)
  :precondition
 ...
 (:action ugly8ugly1ugly2ugly4calibrate :parameters
 ...
 (:action ugly9ugly3ugly9ugly3communicate_image_data :parameters
