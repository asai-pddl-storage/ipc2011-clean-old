...
(define (domain grid) (:requirements :strips :action-costs)
 (:predicates (locked ?x) (key-shape ?k ?s) (open ?x) (holding ?k) (shape ?s)
  (arm-empty) (at-robot ?x) (place ?p) (key ?k) (lock-shape ?x ?s) (at ?r ?x)
  (conn ?x ?y))
 (:functions (total-cost) - number)
 (:action ugly11073ugly1ugly5ugly4ugly2pickup :parameters (?curpos ?key)
  :precondition
  (and (place ?curpos) (key ?key) (at-robot ?curpos) (at ?key ?curpos)
       (arm-empty))
  :effect (and (holding ?key) (not (at ?key ?curpos)) (not (arm-empty))))
 (:action ugly11074ugly4ugly2ugly3ugly1putdown :parameters (?curpos ?key)
  :precondition
  (and (place ?curpos) (key ?key) (at-robot ?curpos) (holding ?key)) :effect
  (and (arm-empty) (at ?key ?curpos) (not (holding ?key))))
 (:action ugly11075ugly2ugly3ugly1ugly5pickup-and-loose :parameters
 ...
 (:action ugly11076ugly5ugly4ugly2ugly3unlock :parameters
 ...
 (:action ugly11077ugly3ugly1ugly5ugly4move :parameters (?curpos ?nextpos)
  :precondition
