...
(define (domain genome-edit-distance) (:requirements :equality :action-costs)
 (:predicates (inverse-splicing) (present ?x) (swappable ?x ?y)
  (splice-next ?x) (cut-point-2 ?x) (free ?x) (have-cut) (duplicate ?x ?y)
  (splice-point-1 ?x) (finished) (normal ?x) (gone ?x) (splice-point-2 ?x)
  (s-first ?x) (inverse-splice-next ?x) (inverse-splicing-last) (idle)
  (cutting) (last-cut-point ?x) (inverted ?x) (cut-point-1 ?x) (cw ?x ?y)
  (splicing) (splicing-last) (s-last ?x) (s-next ?x ?y))
 (:functions (total-cost) - number)
 (:action ugly1ugly18ugly12begin-transpose-splice :parameters (?x ?y)
 ...
 (:action ugly2ugly16ugly7continue-inverse-splice-2 :parameters (?x ?y)
 ...
 (:action ugly3ugly10ugly3begin-transverse-splice :parameters (?x ?y)
  :precondition (and (not (= ?x ?y)) (have-cut) (cw ?x ?y)) :effect
  (and (not (have-cut)) (not (cw ?x ?y)) (inverse-splicing) (splice-point-1 ?x)
       (splice-point-2 ?y) (increase (total-cost) 2)))
 (:action ugly4ugly13ugly9invert-single-gene-a :parameters (?x) :precondition
  (and (idle) (normal ?x)) :effect
  (and (not (normal ?x)) (inverted ?x) (increase (total-cost) 1)))
 (:action ugly5ugly1ugly18begin-inverse-splice-special-case :parameters (?x)
 ...
 (:action ugly6ugly17ugly4continue-inverse-splice-1a :parameters (?x ?y)
 ...
 (:action ugly13ugly9ugly19reset-1 :parameters (?x) :precondition
  (and (finished) (last-cut-point ?x)) :effect
  (and (not (last-cut-point ?x)) (not (finished)) (idle)))
 (:action ugly14ugly11ugly2end-inverse-splice-1a :parameters (?x ?y)
  :precondition
 ...
 (:action ugly15ugly8ugly20continue-inverse-splice-1b :parameters (?x ?y)
  :precondition
 ...
 (:action ugly16ugly7ugly15end-splice-1 :parameters (?x ?y) :precondition
 ...
 (:action ugly20ugly14ugly11continue-cut :parameters (?x ?y) :precondition
  (and (not (= ?x ?y)) (cutting) (s-last ?x) (cw ?x ?y)) :effect
  (and (not (cw ?x ?y)) (not (s-last ?x)) (s-next ?x ?y) (s-last ?y)))
 (:action ugly21ugly6ugly17end-cut-2 :parameters (?x ?y) :precondition
