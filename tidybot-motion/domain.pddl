...
(define (domain tidybot)
 ...
 (:predicates (object-pos ?o - object ?x - xc ?y - yc)
  (base-obstacle ?x - xc ?y - yc) (zerox-rel ?x - xrel)
  (object-done ?o - object) (sum-y ?y - yc ?yr - yrel ?ysum - yc)
  (gripper-obstacle ?x - xc ?y - yc) (holding ?r - robot ?o - object)
  (zeroy-rel ?y - yrel) (object-goal ?o - object ?x - xc ?y - yc)
  (sum-x ?x - xc ?xr - xrel ?xsum - xc) (leftof-rel ?x1 - xrel ?x2 - xrel)
  (above ?y1 - yc ?y2 - yc) (above-rel ?y1 - yrel ?y2 - yrel)
  (base-pos ?r - robot ?x - xc ?y - yc)
  (gripper-rel ?r - robot ?x - xrel ?y - yrel)
  (cart-pos ?c - cart ?x - xc ?y - yc) (surface ?x - xc ?y - yc)
  (on-cart ?o - object ?c - cart) (not-pushed ?c - cart) (parked ?r - robot)
  (leftof ?x1 - xc ?x2 - xc) (not-pushing ?r - robot)
  (gripper-empty ?r - robot) (pushing ?r - robot ?c - cart))
 (:types robot cart object xc yc xrel yrel) (:functions (total-cost) - number)
 (:action ugly11874ugly11879ugly11886ugly23ugly9ugly3ugly20get-up :parameters
 ...
 (:action ugly11875ugly11876ugly11881ugly28ugly14ugly5ugly19put-left
  :parameters
  (?r - robot ?basex - xc ?basey - yc ?gxrel - xrel ?gxabs - xc ?gyrel - yrel
 ...
 (:action ugly11876ugly11881ugly11901ugly15ugly12ugly27ugly2grasp-cart-left
 ...
 (:action ugly11877ugly11902ugly11882ugly2ugly15ugly12ugly27park :parameters
  (?r - robot) :precondition (and (not (parked ?r)) (not-pushing ?r)) :effect
  (and (parked ?r) (increase (total-cost) 1)))
 (:action ugly11878ugly11874ugly11879ugly13ugly6ugly13ugly6gripper-up
  :parameters
 ...
 (:action ugly11879ugly11886ugly11896ugly14ugly5ugly19ugly25get-from-cart
  :parameters
 ...
 (:action ugly11880ugly11898ugly11883ugly27ugly2ugly15ugly12put-up :parameters
  (?r - robot ?basex - xc ?basey - yc ?gxrel - xrel ?gxabs - xc ?gyrel - yrel
   ?gyabs - yc ?o - object ?oy - yc)
  :precondition
  (and (parked ?r) (base-pos ?r ?basex ?basey) (gripper-rel ?r ?gxrel ?gyrel)
 ...
 (:action ugly11881ugly11901ugly11888ugly20ugly22ugly26ugly28gripper-down
  :parameters
  (?r - robot ?basex - xc ?basey - yc ?gxrel - xrel ?gxabs - xc ?cgyrel - yrel
   ?dgyrel - yrel ?cgyabs - yc ?dgyabs - yc)
  :precondition
  ...
  (and (not (gripper-rel ?r ?gxrel ?cgyrel)) (gripper-rel ?r ?gxrel ?dgyrel)
       (not (gripper-obstacle ?gxabs ?cgyabs))
       (gripper-obstacle ?gxabs ?dgyabs)))
 (:action ugly11882ugly11875ugly11876ugly8ugly21ugly17ugly11get-down
  :parameters
  (?r - robot ?basex - xc ?basey - yc ?gxrel - xrel ?gxabs - xc ?gyrel - yrel
   ?gyabs - yc ?o - object ?oy - yc)
  :precondition
  ...
  (and (not (object-pos ?o ?gxabs ?oy)) (not (gripper-obstacle ?gxabs ?oy))
       (not (gripper-empty ?r)) (holding ?r ?o)))
 (:action ugly11883ugly11900ugly11899ugly16ugly1ugly29ugly10grasp-cart-below
  :parameters (?r - robot ?c - cart ?x - xc ?y - yc ?cy - yc) :precondition
  (and (not (parked ?r)) (not-pushed ?c) (base-pos ?r ?x ?y)
       (cart-pos ?c ?x ?cy) (above ?y ?cy) (not-pushing ?r))
  :effect (and (pushing ?r ?c) (not (not-pushing ?r)) (not (not-pushed ?c))))
 (:action ugly11884ugly11878ugly11874ugly6ugly13ugly6ugly13base-down
 ...
 (:action ugly11885ugly11891ugly11890ugly4ugly30ugly4ugly30base-right
  ...
  (and (not (base-pos ?r ?cx ?y)) (base-pos ?r ?dx ?y)
       ...
       (increase (total-cost) 1)))
 (:action ugly11886ugly11896ugly11887ugly11ugly23ugly9ugly3get-left :parameters
  (?r - robot ?basex - xc ?basey - yc ?gxrel - xrel ?gxabs - xc ?gyrel - yrel
   ?gyabs - yc ?o - object ?ox - xc)
  :precondition
  (and (parked ?r) (base-pos ?r ?basex ?basey) (gripper-rel ?r ?gxrel ?gyrel)
  ...
  (and (not (object-pos ?o ?ox ?gyabs)) (not (gripper-obstacle ?ox ?gyabs))
       (not (gripper-empty ?r)) (holding ?r ?o)))
 (:action ugly11887ugly11884ugly11878ugly1ugly29ugly10ugly18base-cart-left
  :parameters
  ...
  (and (not (base-pos ?r ?x1 ?y)) (base-pos ?r ?x2 ?y)
       ...
       (increase (total-cost) 1)))
 (:action ugly11888ugly11893ugly11880ugly25ugly24ugly8ugly21grasp-cart-above
  :parameters (?r - robot ?c - cart ?x - xc ?y - yc ?cy - yc) :precondition
  (and (not (parked ?r)) (not-pushed ?c) (base-pos ?r ?x ?y)
       (cart-pos ?c ?x ?cy) (above ?cy ?y) (not-pushing ?r))
  :effect (and (pushing ?r ?c) (not (not-pushing ?r)) (not (not-pushed ?c))))
 (:action ugly11889ugly11903ugly11894ugly22ugly26ugly28ugly14base-up
  :parameters (?r - robot ?x - xc ?cy - yc ?dy - yc) :precondition
  ...
  (and (not (base-pos ?r ?x ?cy)) (base-pos ?r ?x ?dy)
       (not (base-obstacle ?x ?cy)) (base-obstacle ?x ?dy)
       (increase (total-cost) 1)))
 (:action ugly11890ugly11877ugly11902ugly9ugly3ugly20ugly22put-right
  :parameters
  (?r - robot ?basex - xc ?basey - yc ?gxrel - xrel ?gxabs - xc ?gyrel - yrel
   ?gyabs - yc ?o - object ?ox - xc)
  :precondition
  ...
  (and (not (holding ?r ?o)) (object-pos ?o ?ox ?gyabs)
       (gripper-obstacle ?ox ?gyabs) (gripper-empty ?r)))
 (:action ugly11891ugly11890ugly11877ugly29ugly10ugly18ugly7grasp-cart-right
  :parameters (?r - robot ?c - cart ?x - xc ?y - yc ?cx - xc) :precondition
  (and (not (parked ?r)) (not-pushed ?c) (base-pos ?r ?x ?y)
       (cart-pos ?c ?cx ?y) (leftof ?x ?cx) (not-pushing ?r))
  :effect (and (pushing ?r ?c) (not (not-pushing ?r)) (not (not-pushed ?c))))
 (:action ugly11892ugly11885ugly11891ugly17ugly11ugly23ugly9base-left
  :parameters (?r - robot ?cx - xc ?dx - xc ?y - yc) :precondition
  ...
  (and (not (base-pos ?r ?cx ?y)) (base-pos ?r ?dx ?y)
       (not (base-obstacle ?cx ?y)) (base-obstacle ?dx ?y)
       (increase (total-cost) 1)))
 (:action ugly11893ugly11880ugly11898ugly10ugly18ugly7ugly16unpark :parameters
 ...
 (:action ugly11894ugly11895ugly11897ugly19ugly25ugly24ugly8get-right
  :parameters
  (?r - robot ?basex - xc ?basey - yc ?gxrel - xrel ?gxabs - xc ?gyrel - yrel
   ?gyabs - yc ?o - object ?ox - xc)
  :precondition
  ...
  (and (not (object-pos ?o ?ox ?gyabs)) (not (gripper-obstacle ?ox ?gyabs))
       (not (gripper-empty ?r)) (holding ?r ?o)))
 (:action ugly11895ugly11897ugly11892ugly12ugly27ugly2ugly15gripper-right
  :parameters
 ...
 (:action ugly11896ugly11887ugly11884ugly5ugly19ugly25ugly24base-cart-right
  :parameters
  (?r - robot ?c - cart ?x1 - xc ?x2 - xc ?y - yc ?cx1 - xc ?cx2 - xc ?cy - yc)
  :precondition
  ...
  (and (not (base-pos ?r ?x1 ?y)) (base-pos ?r ?x2 ?y)
       ...
       (not (base-obstacle ?cx1 ?cy)) (base-obstacle ?cx2 ?cy)
       (increase (total-cost) 1)))
 (:action ugly11897ugly11892ugly11885ugly18ugly7ugly16ugly1put-on-cart
  :parameters
  (?r - robot ?x - xc ?y - yc ?gxrel - xrel ?gyrel - yrel ?o - object ?c - cart
   ?cx - xc ?cy - yc)
  :precondition
 ...
 (:action ugly11898ugly11883ugly11900ugly26ugly28ugly14ugly5ungrasp-cart
  :parameters (?r - robot ?c - cart) :precondition (and (pushing ?r ?c))
  :effect (and (not (pushing ?r ?c)) (not-pushing ?r) (not-pushed ?c)))
 (:action ugly11899ugly11889ugly11903ugly21ugly17ugly11ugly23base-cart-up
  :parameters
  ...
  (and (not (base-pos ?r ?x ?y1)) (base-pos ?r ?x ?y2)
       ...
       (increase (total-cost) 1)))
 (:action ugly11900ugly11899ugly11889ugly30ugly4ugly30ugly4finish-object
 ...
 (:action ugly11901ugly11888ugly11893ugly7ugly16ugly1ugly29base-cart-down
  :parameters
  (?r - robot ?c - cart ?x - xc ?y1 - yc ?y2 - yc ?cx - xc ?cy1 - yc ?cy2 - yc)
  :precondition
  ...
  (and (not (base-pos ?r ?x ?y1)) (base-pos ?r ?x ?y2)
       ...
       (not (base-obstacle ?cx ?cy2)) (base-obstacle ?cx ?cy2)
       (increase (total-cost) 1)))
 (:action ugly11902ugly11882ugly11875ugly3ugly20ugly22ugly26put-down
  :parameters
  (?r - robot ?basex - xc ?basey - yc ?gxrel - xrel ?gxabs - xc ?gyrel - yrel
   ?gyabs - yc ?o - object ?oy - yc)
  :precondition
  ...
  (and (not (holding ?r ?o)) (object-pos ?o ?gxabs ?oy)
       (gripper-obstacle ?gxabs ?oy) (gripper-empty ?r)))
 (:action ugly11903ugly11894ugly11895ugly24ugly8ugly21ugly17gripper-left
  :parameters
  (?r - robot ?basex - xc ?basey - yc ?cgxrel - xrel ?dgxrel - xrel ?cgxabs -
   xc ?dgxabs - xc ?gyrel - yrel ?gyabs - yc)
  :precondition
