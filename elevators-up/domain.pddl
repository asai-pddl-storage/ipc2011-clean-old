...
(define (domain elevators-sequencedstrips)
 ...
 (:predicates (passengers ?lift - elevator ?n - count)
  ...
  (boarded ?person - passenger ?lift - elevator) (next ?n1 - count ?n2 - count)
  (can-hold ?lift - elevator ?n - count))
 (:types elevator - object slow-elevator fast-elevator - elevator passenger -
  object count - object)
 (:functions (total-cost) - number)
 (:action ugly5ugly9ugly10ugly8ugly2ugly5ugly4ugly2board :parameters
 ...
 (:action ugly6ugly7ugly5ugly9ugly6ugly1ugly3ugly6move-up-slow :parameters
 ...
 (:action ugly7ugly5ugly9ugly10ugly4ugly2ugly5ugly4move-down-slow :parameters
  (?lift - slow-elevator ?f1 - count ?f2 - count) :precondition
  (and (lift-at ?lift ?f1) (above ?f2 ?f1) (reachable-floor ?lift ?f2)) :effect
  (and (lift-at ?lift ?f2) (not (lift-at ?lift ?f1))))
 (:action ugly8ugly6ugly7ugly5ugly5ugly4ugly2ugly5move-down-fast :parameters
  (?lift - fast-elevator ?f1 - count ?f2 - count) :precondition
  (and (lift-at ?lift ?f1) (above ?f2 ?f1) (reachable-floor ?lift ?f2)) :effect
  (and (lift-at ?lift ?f2) (not (lift-at ?lift ?f1))))
 (:action ugly9ugly10ugly8ugly6ugly3ugly6ugly1ugly3leave :parameters
  (?p - passenger ?lift - elevator ?f - count ?n1 - count ?n2 - count)
  :precondition
  ...
  (and (passenger-at ?p ?f) (not (boarded ?p ?lift))
       (not (passengers ?lift ?n1)) (passengers ?lift ?n2)))
 (:action ugly10ugly8ugly6ugly7ugly1ugly3ugly6ugly1move-up-fast :parameters
  (?lift - fast-elevator ?f1 - count ?f2 - count) :precondition
  (and (lift-at ?lift ?f1) (above ?f1 ?f2) (reachable-floor ?lift ?f2)) :effect
  (and (lift-at ?lift ?f2) (not (lift-at ?lift ?f1))
