...
(define (domain barman) (:requirements :strips :typing :action-costs)
 (:predicates (contains ?c - container ?b - beverage)
  ...
  (next ?l1 ?l2 - level) (empty ?c - container)
  (shaker-empty-level ?s - shaker ?l - level) (unshaked ?s - shaker)
  (handempty ?h - hand) (cocktail-part2 ?c - cocktail ?i - ingredient)
  (shaker-level ?s - shaker ?l - level)
  (cocktail-part1 ?c - cocktail ?i - ingredient) (shaked ?s - shaker)
  (clean ?c - container) (dispenses ?d - dispenser ?i - ingredient)
  (holding ?h - hand ?c - container))
 (:types hand level beverage dispenser container - object ingredient cocktail -
  beverage shot shaker - container)
 (:functions (total-cost) - number)
 (:action ugly1ugly2empty-shot :parameters (?h - hand ?p - shot ?b - beverage)
  :precondition (and (holding ?h ?p) (contains ?p ?b)) :effect
  (and (not (contains ?p ?b)) (empty ?p) (increase (total-cost) 1)))
 (:action ugly2ugly5grasp :parameters (?h - hand ?c - container) :precondition
  (and (ontable ?c) (handempty ?h)) :effect
  (and (not (ontable ?c)) (not (handempty ?h)) (holding ?h ?c)
       (increase (total-cost) 1)))
 (:action ugly3ugly10pour-shot-to-used-shaker :parameters
  ...
  (and (not (contains ?s ?i)) (contains ?d ?i) (empty ?s)
       ...
       (increase (total-cost) 1)))
 (:action ugly4ugly6pour-shot-to-clean-shaker :parameters
  (?s - shot ?i - ingredient ?d - shaker ?h1 - hand ?l ?l1 - level)
  :precondition
 ...
 (:action ugly5ugly1leave :parameters (?h - hand ?c - container) :precondition
  (holding ?h ?c) :effect
  (and (not (holding ?h ?c)) (handempty ?h) (ontable ?c)
       (increase (total-cost) 1)))
 (:action ugly6ugly7shake :parameters
 ...
 (:action ugly7ugly11pour-shaker-to-shot :parameters
 ...
 (:action ugly8ugly4clean-shot :parameters
 ...
 (:action ugly9ugly9empty-shaker :parameters
 ...
 (:action ugly10ugly8refill-shot :parameters
 ...
 (:action ugly11ugly12fill-shot :parameters
  (?s - shot ?i - ingredient ?h1 ?h2 - hand ?d - dispenser) :precondition
  (and (holding ?h1 ?s) (handempty ?h2) (dispenses ?d ?i) (empty ?s)
       (clean ?s))
  :effect
 ...
 (:action ugly12ugly3clean-shaker :parameters (?h1 ?h2 - hand ?s - shaker)
  :precondition (and (holding ?h1 ?s) (handempty ?h2) (empty ?s)) :effect
