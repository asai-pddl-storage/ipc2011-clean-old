...
(define (domain storage-propositional) (:requirements :typing :action-costs)
 (:predicates (connected ?a1 ?a2 - area) (available ?h - hoist)
  (on ?c - crate ?s - storearea) (clear ?s - storearea)
  (compatible ?c1 ?c2 - crate) (in ?x - (either storearea crate) ?p - place)
  (at ?h - hoist ?a - area) (lifting ?h - hoist ?c - crate))
 (:types hoist surface place area - object container depot - place storearea
  transitarea - area area crate - surface)
 (:functions (total-cost) - number)
 (:action ugly3496ugly3499ugly3496ugly3499ugly3496ugly4ugly1ugly3ugly4lift
 ...
 (:action ugly3497ugly3500ugly3498ugly3497ugly3500ugly3ugly4ugly1ugly3go-out
  :parameters (?h - hoist ?from - storearea ?to - transitarea) :precondition
  (and (at ?h ?from) (connected ?from ?to)) :effect
  (and (not (at ?h ?from)) (at ?h ?to) (clear ?from)))
 (:action ugly3498ugly3497ugly3500ugly3498ugly3497ugly5ugly2ugly5ugly2go-in
  :parameters (?h - hoist ?from - transitarea ?to - storearea) :precondition
  (and (at ?h ?from) (connected ?from ?to) (clear ?to)) :effect
  (and (not (at ?h ?from)) (at ?h ?to) (not (clear ?to))))
 (:action ugly3499ugly3496ugly3499ugly3496ugly3499ugly1ugly3ugly4ugly1move
  :parameters (?h - hoist ?from ?to - storearea) :precondition
  (and (at ?h ?from) (clear ?to) (connected ?from ?to)) :effect
  (and (not (at ?h ?from)) (at ?h ?to) (not (clear ?to)) (clear ?from)))
 (:action ugly3500ugly3498ugly3497ugly3500ugly3498ugly2ugly5ugly2ugly5drop
  :parameters (?h - hoist ?c - crate ?a1 - storearea ?a2 - area ?p - place)
  :precondition
