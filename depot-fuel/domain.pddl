...
(define (domain depot) (:requirements :strips :action-costs)
 (:predicates (at ?x ?y) (hoist ?x) (truck ?x) (locatable ?x) (crate ?x)
  (available ?x) (surface ?x) (distributor ?x) (on ?x ?y) (lifting ?x ?y)
  (clear ?x) (pallet ?x) (depot ?x) (in ?x ?y) (place ?x))
 (:functions (total-cost) - number)
 (:action ugly11034ugly11035ugly11038ugly11034ugly2ugly5ugly3ugly1lift
 ...
 (:action ugly11035ugly11038ugly11034ugly11035ugly5ugly3ugly1ugly2unload
  :parameters (?x ?y ?z ?p) :precondition
  (and (hoist ?x) (crate ?y) (truck ?z) (place ?p) (at ?x ?p) (at ?z ?p)
       (available ?x) (in ?y ?z))
  :effect (and (lifting ?x ?y) (not (in ?y ?z)) (not (available ?x))))
 (:action ugly11036ugly11037ugly11036ugly11037ugly3ugly1ugly2ugly5drop
  :parameters (?x ?y ?z ?p) :precondition
  ...
  (and (available ?x) (at ?y ?p) (clear ?y) (on ?y ?z) (not (lifting ?x ?y))
       (not (clear ?z))))
 (:action ugly11037ugly11036ugly11037ugly11036ugly4ugly4ugly4ugly4load
  :parameters (?x ?y ?z ?p) :precondition
  (and (hoist ?x) (crate ?y) (truck ?z) (place ?p) (at ?x ?p) (at ?z ?p)
       (lifting ?x ?y))
  :effect (and (in ?y ?z) (available ?x) (not (lifting ?x ?y))))
 (:action ugly11038ugly11034ugly11035ugly11038ugly1ugly2ugly5ugly3drive
