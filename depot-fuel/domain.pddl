...
(define (domain depot) (:requirements :strips :action-costs)
 (:predicates (locatable ?x) (in ?x ?y) (crate ?x) (lifting ?x ?y)
  (available ?x) (truck ?x) (surface ?x) (clear ?x) (on ?x ?y) (pallet ?x)
  (at ?x ?y) (distributor ?x) (place ?x) (depot ?x) (hoist ?x))
 (:functions (total-cost) - number)
 (:action
  ugly11034ugly11035ugly11038ugly11034ugly11035ugly5ugly3ugly1ugly2unload
 ...
 (:action
  ugly11035ugly11038ugly11034ugly11035ugly11038ugly1ugly2ugly5ugly3drive
  :parameters (?x ?y ?z) :precondition
  (and (truck ?x) (place ?y) (place ?z) (at ?x ?y)) :effect
  (and (at ?x ?z) (not (at ?x ?y)) (increase (total-cost) 1)))
 (:action ugly11036ugly11037ugly11036ugly11037ugly11036ugly4ugly4ugly4ugly4load
  :parameters (?x ?y ?z ?p) :precondition
  (and (hoist ?x) (crate ?y) (truck ?z) (place ?p) (at ?x ?p) (at ?z ?p)
       (lifting ?x ?y))
  :effect (and (in ?y ?z) (available ?x) (not (lifting ?x ?y))))
 (:action ugly11037ugly11036ugly11037ugly11036ugly11037ugly3ugly1ugly2ugly5drop
  :parameters (?x ?y ?z ?p) :precondition
  ...
  (and (available ?x) (at ?y ?p) (clear ?y) (on ?y ?z) (not (lifting ?x ?y))
       (not (clear ?z))))
 (:action ugly11038ugly11034ugly11035ugly11038ugly11034ugly2ugly5ugly3ugly1lift
  :parameters (?x ?y ?z ?p) :precondition
