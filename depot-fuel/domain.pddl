...
(define (domain depot) (:requirements :strips :action-costs)
 (:predicates (available ?x) (place ?x) (at ?x ?y) (truck ?x) (lifting ?x ?y)
  (depot ?x) (hoist ?x) (in ?x ?y) (crate ?x) (distributor ?x) (pallet ?x)
  (surface ?x) (clear ?x) (locatable ?x) (on ?x ?y))
 (:functions (total-cost) - number)
 (:action ugly1ugly2unload :parameters (?x ?y ?z ?p) :precondition
  (and (hoist ?x) (crate ?y) (truck ?z) (place ?p) (at ?x ?p) (at ?z ?p)
       (available ?x) (in ?y ?z))
  :effect (and (lifting ?x ?y) (not (in ?y ?z)) (not (available ?x))))
 (:action ugly2ugly5drop :parameters (?x ?y ?z ?p) :precondition
  (and (hoist ?x) (crate ?y) (surface ?z) (place ?p) (at ?x ?p) (at ?z ?p)
       (clear ?z) (lifting ?x ?y))
  :effect
  (and (available ?x) (at ?y ?p) (clear ?y) (on ?y ?z) (not (lifting ?x ?y))
       (not (clear ?z))))
 (:action ugly3ugly1lift :parameters (?x ?y ?z ?p) :precondition
 ...
 (:action ugly4ugly4load :parameters (?x ?y ?z ?p) :precondition
  (and (hoist ?x) (crate ?y) (truck ?z) (place ?p) (at ?x ?p) (at ?z ?p)
       (lifting ?x ?y))
  :effect (and (in ?y ?z) (available ?x) (not (lifting ?x ?y))))
 (:action ugly5ugly3drive :parameters (?x ?y ?z) :precondition
