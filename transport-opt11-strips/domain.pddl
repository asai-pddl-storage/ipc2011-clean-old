...
(define (domain transport) (:requirements :typing :action-costs)
 (:predicates (at ?x - locatable ?v - location) (in ?x - package ?v - vehicle)
 ...
 (:action ugly1ugly1drive :parameters (?v - vehicle ?l1 ?l2 - location)
  :precondition (and (at ?v ?l1) (road ?l1 ?l2)) :effect
  (and (not (at ?v ?l1)) (at ?v ?l2)
       (increase (total-cost) (road-length ?l1 ?l2))))
 (:action ugly2ugly3pick-up :parameters
 ...
 (:action ugly3ugly2drop :parameters
  (?v - vehicle ?l - location ?p - package ?s1 ?s2 - capacity-number)
  :precondition
