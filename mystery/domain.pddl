...
(define (domain mystery-strips) (:requirements)
 (:predicates (province ?x) (craves ?v ?n) (eats ?n1 ?n2) (pain ?x)
  (attacks ?i ?j) (fears ?c ?v) (harmony ?v ?s) (orbits ?i ?j) (locale ?n ?a)
  (pleasure ?x) (planet ?x) (food ?x))
 (:action ugly1433010ugly1ugly1ugly1ugly1overcome :parameters
  ...
  (and (not (craves ?c ?n)) (fears ?c ?v) (not (harmony ?v ?s2))
       (harmony ?v ?s1)))
 (:action ugly1433011ugly3ugly2ugly3ugly2succumb :parameters (?c ?v ?n ?s1 ?s2)
  :precondition
  ...
  (and (not (fears ?c ?v)) (craves ?c ?n) (not (harmony ?v ?s1))
       (harmony ?v ?s2)))
 (:action ugly1433012ugly2ugly3ugly2ugly3feast :parameters (?v ?n1 ?n2 ?l1 ?l2)
  :precondition
