...
(define (domain rover) (:requirements :typing)
 (:predicates (calibration_target ?i - camera ?o - objective)
  (communicated_soil_data ?w - waypoint)
  (have_image ?r - rover ?o - objective ?m - mode)
  (have_soil_analysis ?r - rover ?w - waypoint)
  (equipped_for_imaging ?r - rover) (communicated_rock_data ?w - waypoint)
  (full ?s - store) (available ?r - rover)
  (have_rock_analysis ?r - rover ?w - waypoint) (at_rock_sample ?w - waypoint)
  (calibrated ?c - camera ?r - rover) (at_soil_sample ?w - waypoint)
  (equipped_for_rock_analysis ?r - rover) (at ?x - rover ?y - waypoint)
  (channel_free ?l - lander) (equipped_for_soil_analysis ?r - rover)
  (communicated_image_data ?o - objective ?m - mode)
  (visible_from ?o - objective ?w - waypoint)
  (at_lander ?x - lander ?y - waypoint) (store_of ?s - store ?r - rover)
  (supports ?c - camera ?m - mode) (visible ?w - waypoint ?p - waypoint)
  (empty ?s - store) (can_traverse ?r - rover ?x - waypoint ?y - waypoint)
  (on_board ?i - camera ?r - rover))
 (:types rover waypoint store camera mode lander objective)
 (:action ugly1sample_soil :parameters (?x - rover ?s - store ?p - waypoint)
  :precondition
 ...
 (:action ugly2drop :parameters (?x - rover ?y - store) :precondition
  (and (store_of ?y ?x) (full ?y)) :effect (and (not (full ?y)) (empty ?y)))
 (:action ugly3communicate_image_data :parameters
 ...
 (:action ugly4calibrate :parameters
 ...
 (:action ugly5communicate_rock_data :parameters
 ...
 (:action ugly6take_image :parameters
 ...
 (:action ugly7communicate_soil_data :parameters
  (?r - rover ?l - lander ?p - waypoint ?x - waypoint ?y - waypoint)
  :precondition
 ...
 (:action ugly8navigate :parameters (?x - rover ?y - waypoint ?z - waypoint)
  :precondition
  (and (can_traverse ?x ?y ?z) (available ?x) (at ?x ?y) (visible ?y ?z))
  :effect (and (not (at ?x ?y)) (at ?x ?z)))
 (:action ugly9sample_rock :parameters (?x - rover ?s - store ?p - waypoint)
  :precondition
