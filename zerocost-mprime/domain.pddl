...
(define (domain mystery-prime-strips)
 ...
 (:predicates (locale ?n ?a) (province ?x) (orbits ?i ?j) (craves ?v ?n)
  (eats ?n1 ?n2) (pleasure ?x) (fears ?c ?v) (harmony ?v ?s) (planet ?x)
  (food ?x) (attacks ?i ?j) (pain ?x))
 (:action ugly11913ugly3ugly2ugly3ugly2succumb :parameters (?c ?v ?n ?s1 ?s2)
  :precondition
  ...
  (and (not (fears ?c ?v)) (craves ?c ?n) (not (harmony ?v ?s1))
       (harmony ?v ?s2)))
 (:action ugly11914ugly2ugly3ugly2ugly3feast :parameters (?v ?n1 ?n2 ?l1 ?l2)
  :precondition
  ...
  (and (not (craves ?v ?n1)) (craves ?v ?n2) (not (locale ?n1 ?l2))
       (locale ?n1 ?l1)))
 (:action ugly11915ugly4ugly4ugly4ugly4drink :parameters
  ...
  (and (not (locale ?n1 ?l11)) (locale ?n1 ?l12) (not (locale ?n2 ?l21))
       (locale ?n2 ?l22)))
 (:action ugly11916ugly1ugly1ugly1ugly1overcome :parameters (?c ?v ?n ?s1 ?s2)
  :precondition
