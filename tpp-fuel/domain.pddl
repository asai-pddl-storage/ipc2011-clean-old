...
(define (domain tpp-propositional)
 ...
 (:predicates (on-sale ?g - goods ?m - market ?l - level)
  (at ?t - truck ?p - place) (connected ?p1 ?p2 - place)
 ...
 (:action ugly11904ugly11905ugly1ugly3ugly2ugly1unload :parameters
  ...
  (and (loaded ?g ?t ?l1) (not (loaded ?g ?t ?l2)) (stored ?g ?l4)
       (not (stored ?g ?l3))))
 (:action ugly11905ugly11904ugly2ugly1ugly3ugly2drive :parameters
  (?t - truck ?from ?to - place) :precondition
  (and (at ?t ?from) (connected ?from ?to)) :effect
  (and (not (at ?t ?from)) (at ?t ?to) (increase (total-cost) 1)))
 (:action ugly11906ugly11906ugly3ugly2ugly1ugly3load :parameters
 ...
 (:action ugly11907ugly11907ugly4ugly4ugly4ugly4buy :parameters
