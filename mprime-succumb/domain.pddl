...
(define (domain mystery-prime-strips)
 ...
 (:predicates (craves ?v ?n) (pain ?x) (locale ?n ?a) (attacks ?i ?j)
  (province ?x) (food ?x) (planet ?x) (pleasure ?x) (orbits ?i ?j)
  (fears ?c ?v) (eats ?n1 ?n2) (harmony ?v ?s))
 (:action ugly11095ugly3ugly2ugly3ugly2succumb :parameters (?c ?v ?n ?s1 ?s2)
  :precondition
 ...
 (:action ugly11096ugly4ugly4ugly4ugly4drink :parameters
  ...
  (and (not (locale ?n1 ?l11)) (locale ?n1 ?l12) (not (locale ?n2 ?l21))
       (locale ?n2 ?l22)))
 (:action ugly11097ugly2ugly3ugly2ugly3feast :parameters (?v ?n1 ?n2 ?l1 ?l2)
  :precondition
  ...
  (and (not (craves ?v ?n1)) (craves ?v ?n2) (not (locale ?n1 ?l2))
       (locale ?n1 ?l1)))
 (:action ugly11098ugly1ugly1ugly1ugly1overcome :parameters (?c ?v ?n ?s1 ?s2)
  :precondition
