...
(define (domain mystery-prime-strips)
 ...
 (:action ugly1ugly1ugly1ugly1overcome :parameters (?c ?v ?n ?s1 ?s2)
  :precondition
  ...
  (and (not (craves ?c ?n)) (fears ?c ?v) (not (harmony ?v ?s2))
       (harmony ?v ?s1)))
 (:action ugly2ugly3ugly2ugly3feast :parameters (?v ?n1 ?n2 ?l1 ?l2)
  :precondition
  ...
  (and (not (craves ?v ?n1)) (craves ?v ?n2) (not (locale ?n1 ?l2))
       (locale ?n1 ?l1)))
 (:action ugly3ugly2ugly3ugly2succumb :parameters (?c ?v ?n ?s1 ?s2)
  :precondition
 ...
 (:action ugly4ugly4ugly4ugly4drink :parameters
