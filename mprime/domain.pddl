...
(define (domain mystery-prime-strips)
 ...
 (:predicates (pleasure ?x) (craves ?v ?n) (attacks ?i ?j) (province ?x)
  (planet ?x) (eats ?n1 ?n2) (harmony ?v ?s) (locale ?n ?a) (pain ?x)
  (fears ?c ?v) (orbits ?i ?j) (food ?x))
 (:action ugly1433006ugly3ugly2ugly3ugly2succumb :parameters (?c ?v ?n ?s1 ?s2)
  :precondition
  ...
  (and (not (fears ?c ?v)) (craves ?c ?n) (not (harmony ?v ?s1))
       (harmony ?v ?s2)))
 (:action ugly1433007ugly4ugly4ugly4ugly4drink :parameters
  ...
  (and (not (locale ?n1 ?l11)) (locale ?n1 ?l12) (not (locale ?n2 ?l21))
       (locale ?n2 ?l22)))
 (:action ugly1433008ugly2ugly3ugly2ugly3feast :parameters (?v ?n1 ?n2 ?l1 ?l2)
  :precondition
  ...
  (and (not (craves ?v ?n1)) (craves ?v ?n2) (not (locale ?n1 ?l2))
       (locale ?n1 ?l1)))
 (:action ugly1433009ugly1ugly1ugly1ugly1overcome :parameters
