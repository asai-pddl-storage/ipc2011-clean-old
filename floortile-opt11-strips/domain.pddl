...
(define (domain floor-tile) (:requirements :typing)
 (:predicates (left ?x - tile ?y - tile) (painted ?x - tile ?c - color)
  (clear ?x - tile) (available-color ?c - color) (up ?x - tile ?y - tile)
  (down ?x - tile ?y - tile) (robot-has ?r - robot ?c - color)
  (robot-at ?r - robot ?x - tile) (right ?x - tile ?y - tile)
  (free-color ?r - robot))
 (:types robot tile color - object) (:functions (total-cost) - number)
 (:action ugly1ugly1change-color :parameters
 ...
 (:action ugly4ugly2right :parameters (?r - robot ?x - tile ?y - tile)
  :precondition (and (robot-at ?r ?x) (right ?y ?x) (clear ?y)) :effect
  (and (robot-at ?r ?y) (not (robot-at ?r ?x)) (clear ?x) (not (clear ?y))
       (increase (total-cost) 1)))
 (:action ugly5ugly3up :parameters (?r - robot ?x - tile ?y - tile)
  :precondition (and (robot-at ?r ?x) (up ?y ?x) (clear ?y)) :effect
  (and (robot-at ?r ?y) (not (robot-at ?r ?x)) (clear ?x) (not (clear ?y))
       (increase (total-cost) 3)))
 (:action ugly6ugly7down :parameters (?r - robot ?x - tile ?y - tile)
  :precondition (and (robot-at ?r ?x) (down ?y ?x) (clear ?y)) :effect
  (and (robot-at ?r ?y) (not (robot-at ?r ?x)) (clear ?x) (not (clear ?y))
       (increase (total-cost) 1)))
 (:action ugly7ugly5paint-down :parameters
  (?r - robot ?y - tile ?x - tile ?c - color) :precondition
