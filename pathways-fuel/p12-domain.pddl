...
(define (domain pathways-propositional)
 ...
 (:predicates (goal16) (possible ?x - molecule)
  ...
  (synthesis-reaction ?x1 ?x2 - molecule) (goal4) (goal11) (goal6)
 ...
 (:action ugly1ugly14dummy-action-7 :parameters () :precondition
  (or (available skp2p1-skp1) (available skp2-skp1p1-cdk2p1p2-cyca)) :effect
  (and (goal7)))
 (:action ugly2ugly19synthesize :parameters (?x1 ?x2 - molecule) :precondition
  (and (synthesis-reaction ?x1 ?x2) (available ?x1)) :effect
  (and (available ?x2)))
 (:action ugly3ugly17dummy-action-10 :parameters () :precondition
 ...
 (:action ugly7ugly18dummy-action-5 :parameters () :precondition
  (or (available skp2-skp1p1-cdk2-cyca) (available cdk2p1-cyce)) :effect
  (and (goal5)))
 (:action ugly8ugly16dummy-action-2 :parameters () :precondition
  (or (available skp2p1) (available cebp-prbp1p2-gp)) :effect (and (goal2)))
 (:action ugly9ugly21dummy-action-8 :parameters () :precondition
  (or (available cdc25a) (available p27-cdk2p2-cyca)) :effect (and (goal8)))
 (:action ugly10ugly20dummy-action-3 :parameters () :precondition
 ...
 (:action ugly14ugly12associate :parameters (?x1 ?x2 - molecule ?x3 - complex)
  :precondition
  (and (association-reaction ?x1 ?x2 ?x3) (available ?x1) (available ?x2))
  :effect
  (and (increase (total-cost) 1) (not (available ?x1)) (not (available ?x2))
       (available ?x3)))
 (:action ugly15ugly9dummy-action-16 :parameters () :precondition
  (or (available c-myc-max) (available prbp1p2-ap2)) :effect (and (goal16)))
 (:action ugly16ugly7dummy-action-13 :parameters () :precondition
  (or (available p27p1-cdk2p2-cyce) (available p27p1-cdk2p1p2-cycep1)) :effect
  (and (goal13)))
 (:action ugly17ugly15dummy-action-4 :parameters () :precondition
  (or (available prbp1p2-ap2-ge-c) (available sl1p1)) :effect (and (goal4)))
 (:action ugly18ugly10dummy-action-15 :parameters () :precondition
  (or (available cebp-prbp1p2) (available p21-cdk2-cyca)) :effect
  (and (goal15)))
 (:action ugly19ugly5choose :parameters (?x - simple ?l1 ?l2 - level)
  :precondition
  (and (possible ?x) (not (chosen ?x)) (num-subs ?l2) (next ?l1 ?l2)) :effect
  (and (chosen ?x) (not (num-subs ?l2)) (num-subs ?l1)))
 (:action ugly20ugly8dummy-action-11 :parameters () :precondition
  (or (available p27p1-cdk2p1-cyca) (available cdk1p3-cks1)) :effect
  (and (goal11)))
 (:action ugly21ugly13dummy-action-1 :parameters () :precondition
