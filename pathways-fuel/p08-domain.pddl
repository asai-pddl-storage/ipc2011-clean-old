...
(define (domain pathways-propositional)
 ...
 (:predicates (goal1) (possible ?x - molecule) (goal5)
  (association-reaction ?x1 ?x2 - molecule ?x3 - complex) (goal7) (goal10)
  (goal12) (chosen ?s - simple) (num-subs ?l - level) (goal8) (goal3) (goal4)
  (goal9) (available ?x - molecule) (synthesis-reaction ?x1 ?x2 - molecule)
  (goal6) (next ?l1 ?l2 - level) (goal2) (goal11)
 ...
 (:action ugly1ugly14initialize :parameters (?x - simple) :precondition
  (and (chosen ?x)) :effect (and (available ?x)))
 (:action ugly2ugly3dummy-action-8 :parameters () :precondition
  (or (available p21-cdk46p1p2-cycd) (available mdm2-prbp1p2)) :effect
  (and (goal8)))
 (:action ugly3ugly13dummy-action-7 :parameters () :precondition
  (or (available skp2-skp1-cdk2-cyca) (available cdk2p1p2-cyce)) :effect
  (and (goal7)))
 (:action ugly4ugly11dummy-action-11 :parameters () :precondition
 ...
 (:action ugly5ugly1dummy-action-9 :parameters () :precondition
  ...
  (and (goal9)))
 (:action ugly6ugly17dummy-action-1 :parameters () :precondition
  (or (available p68p1p2) (available skp2-skp1p1-cdk2-cyca)) :effect
  (and (goal1)))
 (:action ugly7ugly15dummy-action-5 :parameters () :precondition
  (or (available p21-cdk2p1p2-cycep1) (available skp2p1-skp1)) :effect
  (and (goal5)))
 (:action ugly8ugly8dummy-action-3 :parameters () :precondition
  (or (available skp2-skp1p1-cdk2p1-cyca) (available skp2-skp1p1)) :effect
  (and (goal3)))
 (:action ugly9ugly5choose :parameters (?x - simple ?l1 ?l2 - level)
  :precondition
  (and (possible ?x) (not (chosen ?x)) (num-subs ?l2) (next ?l1 ?l2)) :effect
  (and (chosen ?x) (not (num-subs ?l2)) (num-subs ?l1)))
 (:action ugly10ugly9synthesize :parameters (?x1 ?x2 - molecule) :precondition
  (and (synthesis-reaction ?x1 ?x2) (available ?x1)) :effect
  (and (available ?x2)))
 (:action ugly11ugly16associate-with-catalyze :parameters
  (?x1 ?x2 - molecule ?x3 - complex) :precondition
  (and (catalyzed-association-reaction ?x1 ?x2 ?x3) (available ?x1)
       (available ?x2))
  :effect
  (and (increase (total-cost) 1) (not (available ?x1)) (available ?x3)))
 (:action ugly12ugly7dummy-action-10 :parameters () :precondition
  (or (available ercc1) (available hdac1-prb-e2f13p1-dp12p1)) :effect
  (and (goal10)))
 (:action ugly13ugly12dummy-action-2 :parameters () :precondition
  (or (available skp2-skp1p1-cdk2p1p2-cyca)
      (available skp2-skp1p1-cdk2p2-cyca))
  :effect (and (goal2)))
 (:action ugly14ugly2associate :parameters (?x1 ?x2 - molecule ?x3 - complex)
  :precondition
  (and (association-reaction ?x1 ?x2 ?x3) (available ?x1) (available ?x2))
  :effect
  (and (increase (total-cost) 1) (not (available ?x1)) (not (available ?x2))
       (available ?x3)))
 (:action ugly15ugly10dummy-action-4 :parameters () :precondition
  (or (available p53p1-dp12p1) (available skp2p1-skp1p1)) :effect
  (and (goal4)))
 (:action ugly16ugly4dummy-action-6 :parameters () :precondition
  (or (available p53-dp12p1) (available p21-cdk2p1-cyce)) :effect
  (and (goal6)))
 (:action ugly17ugly6dummy-action-12 :parameters () :precondition
