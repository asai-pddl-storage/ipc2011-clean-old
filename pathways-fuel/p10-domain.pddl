...
(define (domain pathways-propositional)
 ...
 (:predicates (synthesis-reaction ?x1 ?x2 - molecule)
  ...
  (next ?l1 ?l2 - level) (goal8) (chosen ?s - simple) (goal11)
  (possible ?x - molecule) (goal4) (goal9) (goal2) (goal12)
  (available ?x - molecule) (goal5) (goal7) (num-subs ?l - level) (goal1)
  (goal6) (goal13)
 ...
 (:action ugly1ugly14associate :parameters (?x1 ?x2 - molecule ?x3 - complex)
  :precondition
  (and (association-reaction ?x1 ?x2 ?x3) (available ?x1) (available ?x2))
  :effect
  (and (increase (total-cost) 1) (not (available ?x1)) (not (available ?x2))
       (available ?x3)))
 (:action ugly2ugly19dummy-action-4 :parameters () :precondition
  (or (available hdac1-p107-e2f4-dp12-ge2) (available c-myc)) :effect
  (and (goal4)))
 (:action ugly3ugly17dummy-action-7 :parameters () :precondition
 ...
 (:action ugly7ugly18dummy-action-8 :parameters () :precondition
  ...
  (and (goal8)))
 (:action ugly8ugly16dummy-action-10 :parameters () :precondition
 ...
 (:action ugly12ugly4dummy-action-6 :parameters () :precondition
  (or (available p27-cdk2-cyce) (available p27-cdk2-cyca)) :effect
  (and (goal6)))
 (:action ugly13ugly7dummy-action-13 :parameters () :precondition
  (or (available p57-cdk2-cycep1) (available cdc25ap1)) :effect (and (goal13)))
 (:action ugly14ugly3dummy-action-12 :parameters () :precondition
  (or (available raf1-cdc25a) (available skp2-cdk2-cyca)) :effect
  (and (goal12)))
 (:action ugly15ugly5choose :parameters (?x - simple ?l1 ?l2 - level)
  :precondition
  (and (possible ?x) (not (chosen ?x)) (num-subs ?l2) (next ?l1 ?l2)) :effect
  (and (chosen ?x) (not (num-subs ?l2)) (num-subs ?l1)))
 (:action ugly16ugly15synthesize :parameters (?x1 ?x2 - molecule) :precondition
  (and (synthesis-reaction ?x1 ?x2) (available ?x1)) :effect
  (and (available ?x2)))
 (:action ugly17ugly12associate-with-catalyze :parameters
  (?x1 ?x2 - molecule ?x3 - complex) :precondition
  (and (catalyzed-association-reaction ?x1 ?x2 ?x3) (available ?x1)
       (available ?x2))
  :effect
  (and (increase (total-cost) 1) (not (available ?x1)) (available ?x3)))
 (:action ugly18ugly13dummy-action-2 :parameters () :precondition
  (or (available p27-cdk46-cycd) (available cyca)) :effect (and (goal2)))
 (:action ugly19ugly9dummy-action-1 :parameters () :precondition
