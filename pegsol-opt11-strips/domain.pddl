...
(define (domain pegsolitaire-sequential) (:requirements :typing :action-costs)
 (:predicates (occupied ?l - location) (free ?l - location)
  (last-visited ?l - location) (in-line ?x ?y ?z - location) (move-ended))
 (:types location - object) (:functions (total-cost) - number)
 (:action ugly1ugly1jump-new-move :parameters
 ...
 (:action ugly2ugly3jump-continue-move :parameters
  (?from - location ?over - location ?to - location) :precondition
  ...
  (and (not (occupied ?from)) (not (occupied ?over)) (not (free ?to))
       ...
       (last-visited ?to)))
 (:action ugly3ugly2end-move :parameters (?loc - location) :precondition
